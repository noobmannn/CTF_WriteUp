...
Đọc qua và debug có thể thấy response được giải mã bằng cách decrypt base64 sau đó tiếp decrypt rc4 bằng cách sủ dụng key là chuỗi hash được tạo từ chuỗi ``FO9xxxxx`` được dùng để mã hoá chuỗi ``ahoy`` lúc gửi đi, sau đó qua thêm một vài bước biến đổi nữa rồi chương trình quay về main để ghép chuỗi giải mã được với ``@flrare-on.com``

Tiến hành sửa lại đoạn code Flask để giả lập lại quá trình host mã hoá thành ciphertext base64 để gửi đi
...
Debug lại và ta lấy được flag của challenge
